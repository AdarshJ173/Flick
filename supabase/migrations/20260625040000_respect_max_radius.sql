-- Drop functions first to update signatures with DEFAULT NULL
DROP FUNCTION IF EXISTS public.get_nearby_signals(DOUBLE PRECISION, DOUBLE PRECISION, INT) CASCADE;
DROP FUNCTION IF EXISTS public.count_nearby_signals(DOUBLE PRECISION, DOUBLE PRECISION, INT) CASCADE;

-- Recreate get_nearby_signals using PL/pgSQL to respect caller's max_radius_m settings
...
BEGIN
  ...
  v_search_radius := COALESCE(
    in_search_radius_m,
    (SELECT max_radius_m FROM public.profiles WHERE id = auth.uid()),
    2000
  );
...
-- Recreate count_nearby_signals using PL/pgSQL to respect caller's max_radius_m settings
