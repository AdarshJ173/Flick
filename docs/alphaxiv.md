...
```mermaid
flowchart LR
    A[Chronic Loneliness] --> B[Social Threat Hypervigilance]
    B --> C[Neutral cues read as hostile]
    C --> D[Defensive/avoidant behavior]
    D --> E[Others perceive coldness]
    E --> F[Actual rejection/social failure]
    F --> A
```
