...
function ProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: p } = await supabase.from("profiles").select("display_name,avatar_emoji,vibe").eq("id", u.user.id).maybeSingle();
      if (p) {
  ...
  return (
    <AppShell>
      <div className="px-5 pt-12">
        <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Profile</div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Only revealed to people you both said yes to.</p>

        <div className="mt-8 flex items-center gap-5">
          <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-surface text-5xl">
            {emoji}
          </div>
          <div className="flex-1">
            <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">Avatar</div>
              ...
              <div className="flex gap-1.5 px-1">
                {EMOJI_CHOICES.map(e => (
                  <button
                    key={e}
                    onClick={() => setEmoji(e)}
                    className={`no-tap flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-xl transition active:scale-90 ${
                      e === emoji ? "bg-primary/20 ring-2 ring-primary/40" : "bg-surface"
                    }`}
                  >{e}</button>
                ))}
              </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          <Field label="Your vibe (one line)" value={vibe} onChange={setVibe} placeholder="Always down for a long walk and weird ideas." />
